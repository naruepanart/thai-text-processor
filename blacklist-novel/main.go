...
func main() {
	// Load replacements from blacklist-nv.json
	blacklistFile, err := os.Open("blacklist-nv.json")
	if err != nil {
		panic(fmt.Errorf("failed to open blacklist-nv.json: %v", err))
	}
	...
	if err := json.NewDecoder(blacklistFile).Decode(&blacklistReplacements); err != nil {
		panic(fmt.Errorf("failed to decode blacklist-nv.json: %v", err))
	}
